...
remain under human control (A detailed architecture is provided in Appendix A.). Decomposing the
...
\begin{figure}[t]
\centerline{\includegraphics[height=2.7in]{architecture.pdf}}
\caption{End-to-end architecture: capture, recognise (VLM + OCR), retrieve
(price, canonical name, candidate images), generate, guardrails, human review,
and publish, with a feedback loop to a labelled store.}
...
\begin{figure}[t]
\centerline{\includegraphics[height=2.7in]{dataflow.pdf}}
...
valuable than maximizing automation---detecting age-restricted items, flagging
...
Figure~\ref{fig:architecture_appendix} presents the complete
Snap-to-Sell architecture.
...
Figure~\ref{fig:dataflow_appendix} illustrates
the artifacts exchanged between pipeline stages.
